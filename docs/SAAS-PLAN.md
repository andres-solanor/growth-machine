...
- **Web:** Next.js 15 App Router, `output: 'standalone'`, one Hostinger managed Node app,
  ...
  **Hosting constraint:** the account has a hard *max processes* cap; when hit, LiteSpeed
  drops TCP connections without an HTTP response (503s / worker `RemoteDisconnected`).
  Rules: no unbounded client polling (progress page polls ≤3 min then stops; email is the
  primary notification), small self-draining MySQL pool (3 conns, 30 s idle timeout) so
  idle node processes get reaped.
- **Worker:** the existing Python engine, headless (JSON payload output), triggered by
  ...
  All HTTP calls to the app retry with exponential backoff (the result POST hardest —
  a dropped connection must not kill a finished analysis).
...
| 2026-06-12 | **Regla anti-límite-de-procesos**: ningún sondeo infinito en el cliente. La página de progreso sondea ACOTADO (3 s el primer ½ min, 10 s hasta los 3 min, luego se detiene con botón "Actualizar estado"); el correo "reporte listo" es el aviso principal. Pestañas ocultas no consultan. Pool MySQL chico que se vacía solo (`connectionLimit: 3, maxIdle: 1, idleTimeout: 30 s`). El runner del worker reintenta HTTP con backoff exponencial (4 intentos; 6 en el POST del resultado) ante 5xx/conexión cortada | hPanel mostró la cuenta tocando el **límite de procesos por cuenta de hosting** (no CPU/RAM): LiteSpeed entonces corta conexiones TCP sin respuesta HTTP → 503 en el sitio. Cadena: el sondeo de 3 s nunca se detenía (ni en éxito/fallo, pestaña abierta = sondeo eterno) y cada proceso node retenía conexiones MySQL ociosas para siempre, así que los procesos nunca se reciclaban. Cualquier feature futura que "espere" algo debe usar correo/aviso o sondeo acotado, nunca un intervalo sin tope. **Nota post-mortem:** los `RemoteDisconnected` del worker resultaron tener OTRA causa: el secreto `APP_BASE_URL` de GitHub seguía apuntando al dominio preview viejo (`*.hostingersite.com`, retirado al adjuntar el dominio real) — corregido a `https://analytikz.com.co` el 2026-06-12. Los reintentos se quedan como blindaje; el workflow `diag-worker-conexion.yml` (manual) distingue secreto-errado vs bloqueo de IPs en el futuro |
| 2026-06-12 | **Prioridad siguiente: primeros clientes reales** (decisión de Andrés en la sesión de revisión). Nada de features nuevas hasta que alguien pague; foco en marketing/outreach con la app actual. Backlog detrás, en orden: delta report → PDF magazine → sección de servicios / categorías con IA | La app ya es vendible: precios publicados, funnel completo, estabilidad arreglada. El riesgo principal ahora no es técnico sino de mercado: validar que alguien paga el precio publicado. Cada feature extra sin clientes es especulación |
